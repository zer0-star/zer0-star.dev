...
topPage = do
  JE.div [ "class" := "h-2/3 flex flex-row gap-0 items-center my-auto" ] do
    JE.div [ "class" := "contain-paint px-2" ] do
      JE.img [ "src" := "/img/zer0-star.svg", "height" := "240", "width" := "240", "alt" := "zer0-star", "class" := "animate-top-left size-32 md:size-60" ]
    JE.div [ "class" := "h-40 md:h-[400px] w-[2px] bg-pale-blue animate-top-line origin-top" ] mempty
    JE.div [ "class" := "flex flex-col gap-6 text-5xl md:gap-20 md:text-8xl animate-top-right contain-paint px-2" ] do
...
navLink page str = hooks do
  ...
  let
    ...
    handleClick e = do
      ...
      useTransition do
        pushUrl url
  pure $ JE.a [ on click handleClick, "href" := urlToString basePath url, "class" := "font-Quicksand font-light tracking-tightest block" ] $ text str
